            ...
            foreach ($technique in $AtomicTechniqueHash) {
                ...
                foreach ($test in $technique.atomic_tests) {
                    ...
                    elseif ($Cleanup) {
                        ...
                        $stopTime = Get-Date
                        if($isLoggingModuleSet) {
                            Write-ExecutionLog $startTime $stopTime $technique $AT $order $test.name $test.auto_generated_guid $test.executor.name $test.description $final_command $ExecutionLogPath $executionHostname $executionUser $res.StandardOutput $res.ErrorOutput (-Not($IsLinux -or $IsMacOS))
                            $order++
                        }
